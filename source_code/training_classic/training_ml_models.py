...
name_response_column = "response"

response = df[name_response_column]
df_data = df.drop(columns=[name_response_column])
...
df_export = pd.DataFrame(matrix_data, columns=['description', 'test_r2_score', 'test_mean_squared_error', 'fit_time', 'score_time', 'train_test_score'])
